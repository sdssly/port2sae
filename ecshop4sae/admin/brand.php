...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'add') {
    ...
    /* 处理图片 */ //for sae
//    $img_name = basename($image->upload_image($_FILES['brand_logo'],'data/brandlogo')); 
    $img_name = $image->upload_image($_FILES['brand_logo'], 'data/brandlogo');
    /* 处理URL */
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'query') {
    ...
    make_json_result($smarty->fetch('brand_list.htm'), '', array('filter' => $brand_list['filter'], 'page_count' => $brand_list['page_count']));
}
...
function get_brandlist() {
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res)) {
        ...
        // for sae
        $brand_logo = empty($rows['brand_logo']) ? '' :
                '<a href="' . sae_repalce_imgpath($rows['brand_logo']) . '" target="_brank"><img src="images/picflag.gif" width="16" height="16" border="0" alt=' . $GLOBALS['_LANG']['brand_logo'] . ' /></a>';
