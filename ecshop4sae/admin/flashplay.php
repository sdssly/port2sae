...
if ($_REQUEST['act'] == 'list') {
    ...
    if (empty($_POST['step'])) {
        ...
        if (!empty($_FILES['img_file_src']['name'])) {
            ...
            $target = saestor(DATA_DIR . '/afficheimg/' . $name); // for sae
    ...
    if (empty($_POST['step'])) {
        ...
        if (!empty($_FILES['img_file_src']['name'])) {
            ...
            $name .= '.' . end(explode('.', $_FILES['img_file_src']['name']));
//            $target = DATA_DIR  . '/afficheimg/' . $name;
            $target = saestor(DATA_DIR . '/afficheimg/' . $name); // for sae
...
//    dump(saestor(DATA_DIR  . '/flash_data.xml'),'xml_path');
...
function put_flash_xml($flashdb) {
    if (!empty($flashdb)) {
        ...
        $xml .= '</bcaster>';
        file_put_contents(saestor(DATA_DIR  . '/flash_data.xml'), $xml);
    } else {
        unlink(saestor(DATA_DIR  . '/flash_data.xml'));
    }
}
...
function get_url_image($url) {
    ...
    $tmp_file = saestor(DATA_DIR . '/afficheimg/' . $name);
    $filename = $tmp_file;
...
function set_flash_uproll($tplname, $flashdata) {
    $data_file = DATA_DIR  . '/flashdata/' . $tplname . '/data.xml';
...
function set_flash_focus($tplname, $flashdata) {
    $data_file = DATA_DIR  . '/flashdata/' . $tplname . '/data.js';
    $jsdata = '';
