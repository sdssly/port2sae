    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            if ($proc_thumb && gd_version() > 0)
            {
                ...
                // for sae
//                sleep(1);
                copy( $img_original,saestor($newname));
                $img_url    = $newname;
//                dump($newname,'newname');
        ...
        elseif (!empty($image_urls[$key]) && ($image_urls[$key] != $GLOBALS['_LANG']['img_file']) && ($image_urls[$key] != 'http://') && copy(trim($image_urls[$key]), saestor('temp/' . basename($image_urls[$key]))))
        {
            $image_url = trim($image_urls[$key]);

            //定义原图路径
            $down_img = SAESTOR . 'temp/' . basename($image_url);
...
function reformat_image_name($type, $goods_id, $source_img, $position='')
{
    ...
    if (!make_dir(SAESTOR.$dir.'/'.$sub_dir))
    {
        return false;
    }
    if (!make_dir(SAESTOR.$dir.'/'.$sub_dir.'/source_img'))
    {
        return false;
    }
    if (!make_dir(SAESTOR.$dir.'/'.$sub_dir.'/goods_img'))
    {
        return false;
    }
    if (!make_dir(SAESTOR.$dir.'/'.$sub_dir.'/thumb_img'))
    {
        return false;
    }
    switch($type)
    ...
    if ($position == 'source')
    {
        if (move_image_file($source_img, SAESTOR.$dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext))
        {
            return SAESTOR.$dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext;
        }
    }
    elseif ($position == 'thumb')
    {
        if (move_image_file($source_img, SAESTOR.$dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext))
        {
            return SAESTOR.$dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext;
        }
    }
    else
    {
        if (move_image_file($source_img, SAESTOR.$dir.'/'.$sub_dir.'/goods_img/'.$img_name.$img_ext))
        {
            return SAESTOR.$dir.'/'.$sub_dir.'/goods_img/'.$img_name.$img_ext;
        }
    }
    return false;
}
