...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    $run_log = DATA_DIR . '/sqldata/run.log';
    ...
    if (empty($tables))
    {
        ...
        {
            /* 有多个文件 */
            if (!@file_put_contents(DATA_DIR . '/sqldata/' . $sql_file_name . '_' . $vol . '.sql', $dump->dump_sql))
            {
        ...
        {
            /* 只有一个文件 */
            if (!@file_put_contents(DATA_DIR . '/sqldata/' . $sql_file_name . '.sql', $dump->dump_sql))
            {
                sys_msg(sprintf($_LANG['fail_write_file'], $sql_file_name . '_' . $vol . '.sql'), 1, array(array('text'=>$_LANG['02_db_manage'], 'href'=>'database.php?act=backup')), false);
            };
...
if ($_REQUEST['act'] == 'upload_sql')
{
    ...
    $sql_file = DATA_DIR . '/upload_database_bak.sql';
