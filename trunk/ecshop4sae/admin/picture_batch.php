...
//                dump($proc_thumb, '$proc_thumb pic batch 144');
                /* 开始处理 */
                if ($proc_thumb) {
...
//                dump($proc_thumb, '$proc_thumb pic batch 151');
...
//                dump($dir,"dir 249");
                $image = $GLOBALS['image']->make_thumb($row['original_img'], $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height'], $dir); //先生成缩略图
//                dump($image,"image");
                if (!$image) {
                    //出错返回
...
function replace_image($new_image, $old_image, $goods_id, $silent) {
    ...
    if (file_exists($old_image)) {
        @rename($old_image, $old_image . '.bak');
        if (!@rename($new_image, $old_image)) {
            $error = true;
        }
    } else {
        if (!@rename($new_image, $old_image)) {
    ...
    if ($error === true) {
        if (file_exists($old_image . '.bak')) {
            @rename($old_image . '.bak', $old_image);
        }
